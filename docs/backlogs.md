...
| 🟡 | Google Books estoura o limite (429) | Achado no navegador de um testador (11/08/2026): a busca de livro chama o Google Books SEM chave, e a cota anônima é por IP — ele devolve `429 Too Many Requests` e a tela mostra "uma das fontes não respondeu" em buscas normais. A Open Library responde, então o resultado aparece, mas o recado de erro vira ruído permanente. Duas saídas: (a) chamar o Google Books só quando a Open Library vier magra — é o que o próprio código já diz que ele é ("FALLBACK de livro"), custa uma ida sequencial em vez de paralela nesses casos e é de graça; (b) cadastrar uma chave da API, que é backlog manual. A (a) resolve a maioria sem pedir nada a você |
| 🟡 | Uma fonte pendurada congela a busca inteira | Medido em 11/08/2026: `searchAll` espera TODAS as fontes e nenhuma tem prazo. Com uma delas sem responder (não falhando — pendurada), a tela fica em "Buscando…" para sempre, mesmo com as outras já prontas. Reproduzido no navegador. O conserto é um teto por fonte (uns 8s) que a trata como "não respondeu", que é o desfecho que a tela já sabe mostrar |
...
| 🟢 | Conferir os painéis no PWA instalado | Rabicho do conserto de 15/08/2026 (decisão 28): a altura da coluna passou a descontar as áreas seguras, e o `Toast` ganhou `.pb-safe`. O que ficou SEM medida é o `Sheet`, que é `fixed` como o toast: o corpo dele tem `pb-8` (32px), que por acaso é quase o indicador de home de um iPhone (34px). Ou seja, funciona por coincidência, e num aparelho com indicador maior o último botão do painel encosta na faixa em que o iOS rouba o primeiro toque. Só dá para decidir vendo num aparelho de verdade — abra um painel de obra no app instalado e veja se o botão de baixo está confortável |
