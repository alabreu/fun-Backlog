...
Entregue em 14/09/2026: **o cartão de link do WhatsApp**, que era tarefa sua de
testar e virou conserto meu. Ele nunca funcionou para JOGO — a prévia
desembrulhava a resposta duas vezes (epílogo da decisão 26). Anime, livro e
filme já funcionavam. Conferido em produção com o link que você mandou: volta
com título, sinopse e capa. **O WhatsApp guarda a prévia por URL**, então o link
que você já enviou pode continuar mostrando a tarja antiga por um tempo — teste
com outra obra.
...
| | Item | Notas |
| --- | --- | --- |
| 🟡 | Mood picker + recomendação | Feature 4, a assinatura do produto. O mood picker não pode ser formulário. Depende da chave do OpenRouter para funcionar de fato — e a Edge Function `llm` também nunca foi implantada (conferido em 14/09/2026: só a `media` existe no projeto). O deploy é meu e leva minutos; ele só faz sentido depois da chave |
...
| 🟢 | Screenshots na ficha | Segunda metade do levantamento de 14/09/2026 (decisão 32), **adiada de propósito** pelo usuário: primeiro ver como o trailer se comporta. IGDB e TMDB têm galeria (`screenshots` e `images.backdrops`); anime e livro não têm. O que já está pronto: a CSP (`img-src ... https:`) deixa passar, e o `core/media/image.ts` já sabe ampliar URL das duas. O que falta: o `ImageViewer` mostra UMA imagem, então precisa de anterior/próxima para virar galeria. O risco é peso — as duas fontes têm variante de tamanho na própria URL, então dá para controlar, mas precisa medir antes |
| 🟢 | Conferir o trailer no aparelho | Entregue em 14/09/2026 (decisão 32) e medido só no navegador, com `img.youtube.com` servido localmente (a política de egresso daqui bloqueia YouTube, IGDB, TMDB e AniList). Falta ver de verdade: se a miniatura fica macia num aparelho 3x — se ficar, o degrau é pedir a `maxresdefault` e cair para a atual quando ela faltar —, e se o trailer que aparece é o certo em jogo (a IGDB não tem campo de tipo, só o nome do vídeo) |
| 🟢 | Conferir teclado e painéis no aparelho | Duas coisas medidas no navegador com `visualViewport` de mentira (decisão 29) e ainda não vistas num iPhone: (1) a busca dentro da estante com o teclado aberto, que era o bug relatado; (2) os painéis com campo de texto — adicionar à mão e a ficha da obra —, que mudaram junto para não quebrarem. Segue sem medida o respiro do `Sheet` sobre o indicador de home: o corpo tem `pb-8` (32px), quase o indicador de um iPhone (34px), ou seja funciona por coincidência, e num aparelho com indicador maior o último botão encosta na faixa em que o iOS rouba o primeiro toque |
